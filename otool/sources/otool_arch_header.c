...
static size_t
otool_arch_obj_name(struct ar_hdr *obj_hdr, char **name, size_t *len)
{
	size_t ext;
...
static int
otool_arch_obj(struct ar_hdr *obj_hdr, const struct macho_info *macho, size_t size)
{
	...
	ext = otool_arch_obj_name(obj_hdr, &name, &len);
	...
	buf_in(&obj, name, len);
	buf_in(&obj, "):\n", 3);
	return (otool_check_header(&obj));
}

static void
otool_arch_name(struct macho_info *macho)
{
	macho->buf_index = 0;
	buf_in(macho, "Archive : ", 10);
	buf_in(macho, macho->file, ft_strlen(macho->file));
	buf_in(macho, "\n", 1);
	buf_out(macho);
}
...
otool_arch_header(struct macho_info *macho)
{
	...
	while ((void *)obj_hdr < macho->ptr + macho->size) {
		...
		err = otool_arch_obj(obj_hdr, macho, obj_size);
		if (err < 0)
			return (-1);
		if (err)
			ret = 1;
